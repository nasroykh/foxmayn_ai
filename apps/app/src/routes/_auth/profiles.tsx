	...
	const createMutation = useMutation(
		orpc.profiles.createProfile.mutationOptions({
			onSuccess: () => {
				...
				queryClient.invalidateQueries({ queryKey: orpc.profiles.listProfiles.key() });
			},
		...
		<Layout>
			<div className="space-y-6">
				<div className="flex items-center justify-between">
					<div>
						...
						<p className="text-muted-foreground text-sm">Configure RAG pipeline settings for different use cases</p>
					</div>
							...
							<Card key={profile.id} className="relative">
								...
								<CardContent className="space-y-1.5">
									...
									<div className="flex justify-between text-xs">
										...
										<span>{profile.chunkSize} / overlap {profile.chunkOverlap}</span>
									</div>
									<div className="flex justify-between text-xs">
			...
			{editingProfile && (
				<ProfileFormDialog
					...
					onSubmit={(values) => updateMutation.mutate({ id: editingProfile.id, data: values })}
			...
			<AlertDialog open={!!deletingProfileId} onOpenChange={() => setDeletingProfileId(null)}>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							Documents linked to this profile will not be deleted, but the profile settings will be removed.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
						...
						<AlertDialogAction
							...
							onClick={() => deletingProfileId && deleteMutation.mutate({ id: deletingProfileId })}
						>
...
function ProfileFormDialog({
	...
	const { register, handleSubmit } = useForm<ProfileFormValues>({
		defaultValues: {
			...
			model: defaultValues?.model ?? "google/gemini-2.5-flash-lite",
			embeddingModel: defaultValues?.embeddingModel ?? "openai/text-embedding-3-small",
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
				<DialogHeader>
					...
					<DialogDescription>Configure the RAG pipeline settings</DialogDescription>
				</DialogHeader>
				<form onSubmit={handleSubmit(onSubmit)} className="space-y-4 py-4">
					<div className="grid grid-cols-2 gap-4">
						...
						<div className="col-span-2 space-y-2">
							...
							<Input {...register("model")} placeholder="google/gemini-2.5-flash-lite" />
						</div>
						<div className="col-span-2 space-y-2">
							<Label>Embedding Model</Label>
							<Input {...register("embeddingModel")} placeholder="openai/text-embedding-3-small" />
						</div>
						<div className="space-y-2">
							<Label>Chunk Size</Label>
							<Input {...register("chunkSize", { valueAsNumber: true })} type="number" />
						</div>
						<div className="space-y-2">
							<Label>Chunk Overlap</Label>
							<Input {...register("chunkOverlap", { valueAsNumber: true })} type="number" />
						</div>
						<div className="space-y-2">
							<Label>Top-K Results</Label>
							<Input {...register("topK", { valueAsNumber: true })} type="number" />
						</div>
						<div className="space-y-2">
							<Label>Temperature</Label>
							<Input {...register("temperature", { valueAsNumber: true })} type="number" step="0.1" min="0" max="2" />
						</div>
						<div className="col-span-2 space-y-2">
						...
						<div className="col-span-2 flex items-center gap-2">
							<input {...register("isDefault")} type="checkbox" id="isDefault" className="size-4" />
